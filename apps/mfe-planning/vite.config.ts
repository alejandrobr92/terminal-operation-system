...
export default defineConfig(({ mode }) => {
  ...
  const remotePublicUrl =
    process.env.VITE_REMOTE_PUBLIC_URL ?? env.VITE_REMOTE_PUBLIC_URL ?? "http://127.0.0.1:3002";

  if (mode !== "development" && !process.env.VITE_REMOTE_PUBLIC_URL) {
    throw new Error("Production planning builds require VITE_REMOTE_PUBLIC_URL.");
  }
